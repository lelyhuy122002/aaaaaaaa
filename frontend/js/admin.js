...
function openPreviewModal(orderId) {
  ...
  // Display mockup design overlay
  const overlay = document.getElementById('mockupDesignAdmin');
  if (overlay) {
    if (order.designUrl) {
      overlay.innerHTML = `<img src="${order.designUrl}" alt="AI Custom Design">`;
    } else {
      overlay.innerHTML = `<span style="font-size: 11px; color: var(--text-secondary);">No design</span>`;
    }
  }
